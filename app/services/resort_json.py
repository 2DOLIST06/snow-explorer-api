...
def apply_record(resort, record):
        ...
        for key, value in data.items():
            if block == "pistes" and key in {"small_map_url", "large_map_url", "caption"}:
                ...
                if getattr(resort, attr) != value:
                    setattr(resort, attr, value); resort.updated_at = utcnow(); resort.save(); relation_updates.append(f"pistes.{key}")
                continue
            if block == "snowpark" and key in {"map_url", "caption"}:
                attr = "snowpark_" + key
                if getattr(resort, attr) != value:
                    setattr(resort, attr, value); resort.updated_at = utcnow(); resort.save(); relation_updates.append(f"snowpark.{key}")
                continue
    ...
    if row:
        serialized = StationWidgets.to_json(cfg)
        if StationWidgets.from_json(row.config) != cfg:
            row.config = serialized; row.updated_at = utcnow(); row.save()
